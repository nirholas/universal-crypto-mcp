...
export function Screener({ coins }: { coins: Coin[] }) {
  ...
  const SortHeader = ({ field, children }: { field: SortField; children: React.ReactNode }) => (
    <th
      className="px-4 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider cursor-pointer hover:bg-surface-alt select-none"
      onClick={() => handleSort(field)}
    >
      <div className="flex items-center gap-1">
        {children}
        <ChevronUpDownIcon
          className={`w-4 h-4 ${sortField === field ? 'text-text-primary' : ''}`}
        />
        {sortField === field &&
          (sortDirection === 'asc' ? (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 border border-surface-border rounded-lg bg-surface text-text-primary focus:ring-2 focus:ring-primary focus:border-transparent"
          />
        </div>
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg border transition-colors ${
            showFilters || activeFilterCount > 0
              ...
              : 'border-surface-border hover:bg-surface-alt'
          }`}
        >
          <FunnelIcon className="w-5 h-5" />
          Filters
          {activeFilterCount > 0 && (
            <span className="ml-1 px-2 py-0.5 text-xs rounded-full bg-surface text-text-primary">
              {activeFilterCount}
            </span>
          )}
        </button>
      ...
      <div className="flex flex-wrap gap-2">
        {PRESET_FILTERS.map((preset) => (
          <button
            ...
            className="px-3 py-1.5 text-sm rounded-full border border-surface-border hover:bg-surface-alt transition-colors"
          >
            {preset.name}
          </button>
        ))}
      ...
      {/* Filter Panel */}
      {showFilters && (
        <div className="p-4 border border-surface-border rounded-lg bg-surface-alt">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            {/* Market Cap */}
            <div>
              <label className="block text-xs font-medium text-text-muted mb-1">Market Cap</label>
              <div className="flex gap-2">
                <input
                  ...
                  className="w-full px-2 py-1.5 text-sm border border-surface-border rounded bg-surface"
                />
                <input
                  type="number"
                  placeholder="Max"
            ...
            {/* Volume */}
            <div>
              <label className="block text-xs font-medium text-text-muted mb-1">24h Volume</label>
              <div className="flex gap-2">
                <input
                  type="number"
                  placeholder="Min"
            ...
            {/* 24h Change */}
            <div>
              <label className="block text-xs font-medium text-text-muted mb-1">24h Change %</label>
              <div className="flex gap-2">
                <input
                  type="number"
                  placeholder="Min"
            ...
            {/* ATH Distance */}
            <div>
              <label className="block text-xs font-medium text-text-muted mb-1">From ATH %</label>
              <div className="flex gap-2">
                <input
                  type="number"
                  placeholder="Min"
      ...
      <div className="overflow-x-auto scrollbar-hide border border-surface-border rounded-lg">
        <table className="min-w-full divide-y divide-surface-border">
          <thead className="bg-surface-alt">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider w-10">
          ...
          <tbody className="bg-surface divide-y divide-surface-border">
            {filteredCoins.map((coin) => (
              <tr key={coin.id} className="hover:bg-surface-alt">
                <td className="px-4 py-3">
                  ...
                  >
                    <StarIcon
                      className={`w-4 h-4 ${watchlist.has(coin.id) ? 'text-text-primary fill-current' : ''}`}
                    />
                  </button>
                </td>
                <td className="px-4 py-3 text-sm text-text-muted">{coin.market_cap_rank}</td>
                <td className="px-4 py-3">
                ...
                <td
                  className={`px-4 py-3 text-sm font-mono ${
                    coin.price_change_percentage_24h >= 0 ? 'text-text-primary' : 'text-text-muted'
                  }`}
                >
