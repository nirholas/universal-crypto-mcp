...
function generateBreadcrumbs(
  ...
  for (const segment of segments) {
    ...
    const label =
      customLabels[decodedSegment] || defaultLabels[decodedSegment] || toTitleCase(decodedSegment);
...
export function Breadcrumbs({
  ...
  return (
    <nav aria-label="Breadcrumb" className={`py-3 px-4 lg:px-6 max-w-7xl mx-auto ${className}`}>
      <ol
        ...
        {breadcrumbs.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              {!isFirst && (
                <svg
                  className="w-4 h-4 mx-1 text-text-muted flex-shrink-0"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
              ...
              {isLast ? (
                ...
                <Link
                  href={item.href}
                  className="group flex items-center gap-1.5 text-text-muted hover:text-primary transition-colors"
                  itemProp="item"
                >
                  {/* Home icon */}
                  ...
                  <span
                    itemProp="name"
                    className={isFirst && showHomeIcon ? 'sr-only sm:not-sr-only' : ''}
                  >
                    {item.label}
                  </span>
                </Link>
